...
def save_detailed_predictions(output_dir, scaler, y_pred_arima, y_pred_xgb, y_pred_prophet, y_pred_lstm):
    ...
    for ticker in tickers:
        ...
        predictions = pd.DataFrame({
            'Hisse Senedi': ticker,
            'Tarih': test.index.strftime('%d/%m/%Y'),
            'Gerçek Değer': y_test_raw.values,
        ...
        # Grafik oluşturma kodu buraya gelecek (değişiklik yok)
        
    # Tüm tahminleri birleştirme
    all_predictions_df = pd.concat(all_predictions, ignore_index=True)
    
    # İstenen sıraya göre hisse senetlerini düzenle
    desired_order = ["AAPL", "GOOGL", "MSFT", "AMZN", "NVDA"]
    all_predictions_df['Hisse Senedi'] = pd.Categorical(all_predictions_df['Hisse Senedi'], categories=desired_order, ordered=True)
    
    # Hisse senedi ve tarihe göre sıralama
    ...
    with pd.ExcelWriter(excel_path, engine='openpyxl') as writer:
        all_predictions_df.to_excel(writer, sheet_name='Tüm Tahminler', index=False)
